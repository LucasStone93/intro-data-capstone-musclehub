...
Chi2, pval, dof, expected = chi2_contingency(X, labels)
...
member_grouping = just_apps.groupby([just_apps.is_member, just_apps.ab_test_group]).first_name.count().reset_index()
member_pivot = member_grouping.pivot(
    columns = 'is_member',
    index = 'ab_test_group',
    values = 'first_name'
).reset_index()
...
chi2, pval, df, expected = chi2_contingency(X, labels)
print (pval)
print chi2_contingency(X)
...
ax.set_xticklabels(['With fitness test', 'Without fitness test'])
ax.set_yticklabels(['0%', '2%', '4%', '6%', '8%', '10%', '12%'])
plt.title('Proportion of visitors who apply')
plt.show()
...
plt.title('Proportion of appliers who memberise')
plt.show()
...
plt.title('Proportion of visitors who memberise')
plt.show()
